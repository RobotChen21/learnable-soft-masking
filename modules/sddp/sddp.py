...
class StructuredDialogueDiscourseParser:
    ...
    def parse(self, batch):
        ...
        with torch.no_grad():
            ...
            self.model.inference_forward(
                struct_vec.to(self.device), batch[3].to(self.device), self.max_num_contexts
            )
        tree_results = []
        relation_types = []
        ...
        output = []
        for ds, relations in zip(tree_results, relation_types):
            all_d = set()
            for d in ds:
                d = set((head, index + 1, relations[head][index + 1]) for index, head in enumerate(d[1:]))
                all_d.update(d)
            output.append(all_d)
        return output
