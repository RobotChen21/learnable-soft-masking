...
EXPERT_FUSION_MODES = ('original_hard', 'learnable_soft')


def _to_top1_edge(edge):
    """Normalize legacy triples and Top-K parser tuples to one typed edge."""
    if len(edge) == 3:
        head, tail, relation_id = edge
    elif len(edge) == 4:
        head, tail, relation_ids, _ = edge
        if len(relation_ids) == 0:
            raise ValueError("Top-K parsed edge must contain at least one relation")
        relation_id = relation_ids[0]
    else:
        raise ValueError(f"Unsupported parsed edge format with {len(edge)} fields")
    return int(head), int(tail), int(relation_id)


class SoftRGATConv(MessagePassing):
    def __init__(
        self,
        in_channels,
        out_channels,
        num_relations,
        prior_strength_init=1.0,
        learnable_prior=True,
        **kwargs,
    ):
        super(SoftRGATConv, self).__init__(aggr='add', node_dim=0, **kwargs)
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.num_relations = num_relations
        
        self.weight = nn.Parameter(torch.Tensor(num_relations, in_channels, out_channels))
        self.att = nn.Parameter(torch.Tensor(num_relations, 2 * out_channels, 1))
        self.root = nn.Parameter(torch.Tensor(in_channels, out_channels))
        self.bias = nn.Parameter(torch.Tensor(out_channels))
        
        # Expert relations initialized to 1.0 (~73% confidence)
        # Non-expert relation (Special, index 16) initialized to -1.0 (~27% confidence)
        init_lambda = torch.ones(num_relations) * prior_strength_init
        init_lambda[16] = -1.0
        self.lambda_prior = nn.Parameter(init_lambda, requires_grad=learnable_prior)
    ...
    def forward(self, x, edge_index, edge_type, edge_prior, return_attention_weights=False):
        # x: [N, in_channels]
        # 1. Pre-calculate node projections for all relations: [R, N, out_channels]
        x_projected = torch.matmul(x.unsqueeze(0), self.weight) 
        
        # 2. Pre-calculate attention score components for left and right
        att_l = self.att[:, :self.out_channels, :].squeeze(-1) # [R, out_channels]
        att_r = self.att[:, self.out_channels:, :].squeeze(-1) # [R, out_channels]
        
        # x_l, x_r: [R, N]
        x_l = (x_projected * att_l.unsqueeze(1)).sum(dim=-1)
        x_r = (x_projected * att_r.unsqueeze(1)).sum(dim=-1)

        # Explicitly set size=(x.size(0), x.size(0)) to prevent dimension mismatch
        # when the last node in the batch has no incoming edges.
        out = self.propagate(edge_index, x_projected=x_projected, x_l=x_l, x_r=x_r, 
                             edge_type=edge_type, edge_prior=edge_prior, size=(x.size(0), x.size(0)))
    ...
    def message(self, x_projected, x_l, x_r, edge_type, edge_prior, index, ptr, size_i, edge_index_i, edge_index_j):
        # Vectorized extraction of projected features per edge
        x_j_w = x_projected[edge_type, edge_index_j] # [E, out_channels]
        
        # Compute attention score: score = a_l*x_i + a_r*x_j
        alpha = x_l[edge_type, edge_index_i] + x_r[edge_type, edge_index_j] # [E]
        
        alpha = F.leaky_relu(alpha, 0.2)
        
        # Gate Fusion Mechanism: confidence-based balancing
        # gate values in (0, 1) based on relation-specific lambda_prior
        gate = torch.sigmoid(self.lambda_prior[edge_type])
        
        # Mix the model's self-attention score with the expert prior score
        # Initial gate value will prioritize expert prior
        alpha = (1 - gate) * alpha + gate * edge_prior
        
        alpha = softmax(alpha, index, ptr, size_i)
        self._alpha = alpha
        return x_j_w * alpha.unsqueeze(-1)
...
class GATLayer(nn.Module):
    def __init__(
        ...
        num_relations,
        dropout,
        expert_fusion='original_hard',
        prior_strength_init=1.0,
    ):
        super(GATLayer, self).__init__()
        if expert_fusion not in EXPERT_FUSION_MODES:
            raise ValueError(f"Unsupported expert fusion mode: {expert_fusion}")
        self.expert_fusion = expert_fusion
        self.use_soft_masking = expert_fusion != 'original_hard'
        if self.use_soft_masking:
            self.conv = SoftRGATConv(
                in_channels=dim_in,
    ...
    def forward(self, x, edge_index, edge_type, edge_prior=None):
        _x = x
        if self.use_soft_masking:
            x, attention_weights = self.conv(x, edge_index, edge_type, edge_prior=edge_prior, return_attention_weights=True)
        else:
...
class RobertaHeterogeneousGraph(RobertaBase):
    def __init__(self, args, lightmode=True):
        ...
        self.expert_fusion = getattr(args, 'expert_fusion', 'learnable_soft')
        if self.expert_fusion not in EXPERT_FUSION_MODES:
            raise ValueError(f"Unsupported expert fusion mode: {self.expert_fusion}")
        self.use_soft_masking = self.expert_fusion != 'original_hard'
        self.prior_strength_init = getattr(args, 'prior_strength_init', 1.0)
        self.conv1 = GATLayer(dim_in=graph_dim, dim_out=graph_dim, num_relations=len(self.graph_relation_dict.keys()),
                              dropout=0.2, expert_fusion=self.expert_fusion, prior_strength_init=self.prior_strength_init)
    ...
    def forward(self, samples):
        ...
        for i in range(len(samples["dialogue_history"])):
            ...
            parsed_top1_edges = [_to_top1_edge(edge) for edge in parsed_dialogues[i]]
            edges, edge_types, edge_priors = build_discourse_edges(
                len(nodes), parsed_top1_edges, self.expert_fusion,
                self.graph_relation_dict["Special"],
            )
            ...
            graph_inputs["edge_priors"].extend(edge_priors)
        ...
        graph_inputs["edge_priors"] = torch.tensor(graph_inputs["edge_priors"]).to(self.device)
        
        # Graph Layers
        graph_embeddings, atten_weights_1 = self.conv1(graph_inputs["embeddings"], graph_inputs["edges"],
                                                       graph_inputs["edge_types"], graph_inputs["edge_priors"])
